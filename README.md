...
1. Crear y desplegar Google Apps Script como Web App usando:
  - `inscripciones_IM/google_apps_script/Code.gs`
2. Copiar la URL `.../exec` del despliegue.
3. Editar `inscripciones_IM/web_config.js` y completar:
  - `sheetsAppendUrl`
  - `appSecret` (opcional)
4. Publicar `inscripciones_IM/` en tu hosting web.

Si más adelante cambiás la planilla destino, el orden correcto es:

1. Editar `inscripciones_IM/google_apps_script/Code.gs` y cambiar:
...
3. Copiar la nueva URL `.../exec` en `inscripciones_IM/web_config.js` -> `sheetsAppendUrl`.
...
### Catálogo inicial IM (desde CSV)

- El catálogo base se genera desde `inscripciones_IM/materias_iniciacion.csv`.
- El cupo se toma de la columna `Alumnos`.
- Si `Alumnos` está vacía, la comisión se considera grupal (sin límite de cupo).
...
Si no usas Apps Script, podés usar `python inscripciones_IM/server.py`.
